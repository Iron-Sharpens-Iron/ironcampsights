...
class SectionCubit extends Cubit<SectionState> {
  ...
  void getSection({required BuildContext context, required String langId, required String userId, String? latitude, String? longitude}) async {
    try {
      emit(SectionFetchInProgress());
      final result = await _sectionRepository.getSection(context: context, langId: langId, userId: userId, latitude: latitude, longitude: longitude);
      emit(
        SectionFetchSuccess(section: result['Section']),
      );
      print(result);
    } catch (e) {
