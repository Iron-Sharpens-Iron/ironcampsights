...
class SubCategoryScreen extends StatefulWidget {
  ...
  const SubCategoryScreen({super.key, required this.catId, required this.catName, required this.from});
  ...
  static Route route(RouteSettings routeSettings) {
    ...
    return CupertinoPageRoute(builder: (_) => SubCategoryScreen(catId: arguments['catId'], catName: arguments['catName'], from: arguments['from']));
  }
}
...
class SubCategoryScreenState extends State<SubCategoryScreen> with TickerProviderStateMixin {
  ...
  setAppBar() {
    return PreferredSize(
        ...
        child: AppBar(
          ...
          bottom: PreferredSize(preferredSize: Size(MediaQuery.of(context).size.width, 30), child: catData()),
        ));
  }
  setAppBarWithouBack() {
    return PreferredSize(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: widget.from != "dash" ? setAppBar() : setAppBarWithouBack(),
