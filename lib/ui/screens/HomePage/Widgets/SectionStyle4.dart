...
class Style4Section extends StatelessWidget {
  ...
  Widget style4Data(FeatureSectionModel model, BuildContext context) {
    if (model.breakVideos!.isNotEmpty || model.breakNews!.isNotEmpty || model.videos!.isNotEmpty || model.news!.isNotEmpty) {
      // Filter news items for the category "Blogs"
      var blogNews = model.news!.where((news) => news.categoryName == 'Blogs').toList();
      var blogVideos = model.videos!.where((news) => news.categoryName == 'Blogs').toList();
      ...
      return Column(
        ...
        children: [
          ...
          if (model.newsType == 'news' || model.videosType == "news" || model.newsType == "user_choice")
            if ((model.newsType == 'news' || model.newsType == "user_choice") ? blogNews.isNotEmpty : blogVideos.isNotEmpty)
              setGridLayout(
                  context: context,
                  totalCount: (model.newsType == 'news' || model.newsType == "user_choice") ? blogNews.length : blogVideos.length,
                  childWidget: (context, index) {
                    NewsModel data = (model.newsType == 'news' || model.newsType == "user_choice") ? blogNews[index] : blogVideos[index];
                    return InkWell(
                      child: Container(
