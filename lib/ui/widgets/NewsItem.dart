...
import '../screens/NewsDetail/Widgets/descView.dart';
...
class NewsItemState extends State<NewsItem> {
  ...
  Widget setNewsImage() {
    if(widget.model.categoryName=="Challenges"){
      return ClipRRect(
          ...
          child: ShaderMask(
              ...
              child: _buildChallengeItem(widget.model, context)));
    }else{
      return ClipRRect(
          borderRadius: BorderRadius.circular(10),
          child: ShaderMask(
      ...
      child: Column(
        ...
        children: [
          Container(
              ...
              child: CustomTextLabel(
                  text: widget.model.title!, textStyle: Theme.of(context).textTheme.titleSmall?.copyWith(color: textColor), maxLines: 2, softWrap: true, overflow: TextOverflow.ellipsis)),
          Padding(
              padding: const EdgeInsetsDirectional.only(top: 4.0, start: 5.0, end: 5.0),
              child: CustomTextLabel(
                  text: UiUtils.convertToAgo(context, DateTime.parse(widget.model.date!), 0)!, textStyle: Theme.of(context).textTheme.bodySmall!.copyWith(color: textColor.withOpacity(0.6)))),
        ],
      ),
    );
  }
                        ...
                        onTap: () {
                          if (context.read<AuthCubit>().getUserId() != "0") {
                            ...
                            context
                                ...
                                .setLikeAndDisLikeNews(context: context, userId: context.read<AuthCubit>().getUserId(), news: widget.model, status: (isLike) ? "0" : "1");
                          } else {
                            loginRequired(context);
                          }
                        },
