  ...
  getWeatherData() async {
    ...
    if (context.read<AppConfigurationCubit>().getLocationWiseNewsMode() == "1") {
      ...
      if (context.read<SettingsCubit>().getSettings().token != '') {
        ...
        context.read<SettingsCubit>().changeToken(context.read<SettingsCubit>().getSettings().token);
      }
    } else {
      SettingsLocalDataRepository().setLocationCityKeys(null, null);
    }
  ...
  Widget sectionData(int index, FeatureSectionModel model) {
    return Column(
      ...
      children: [
        ...
        if (model.styleApp == 'style_2') Style2Section(model: model, position: index),
        if (model.styleApp == 'style_3') Style3Section(model: model, position: index),
