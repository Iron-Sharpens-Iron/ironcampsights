...
class Style6SectionState extends State<Style6Section> {
  ...
  void initState() {
    super.initState();
    getSectionDataById();
    style6ScrollController.addListener(() {
      if (style6ScrollController.offset >= style6ScrollController.position.maxScrollExtent && style6ScrollController.position.outOfRange) {
        ...
        child: Column(
          ...
          children: [
            if (model.newsType == 'news' || model.videosType == "news" || model.newsType == "user_choice")
              if ((model.newsType == 'news' || model.newsType == "user_choice") ? model.news!.isNotEmpty : model.videos!.isNotEmpty) style6NewsData(model),
            if (model.newsType == 'breaking_news' || model.videosType == "breaking_news")
              if (model.newsType == 'breaking_news' ? model.breakNews!.isNotEmpty : model.breakVideos!.isNotEmpty) style6BreakNewsData(model)
          ],
        ),
      );
    } else {
  ...
  Widget style6NewsData(FeatureSectionModel model) {
    return SizedBox(
              ...
              builder: (context, state) {
                if (state is SectionByIdFetchSuccess) {
                  return Row(
                    ...
                    NewsModel data = state.newsModel[index];
                    return SizedBox(
                      width: 180,
                      child: Padding(padding: const EdgeInsets.all(5), child: setImageCard(index: index, model: model, newsModel: data, allNewsList: state.newsModel)),
                    );
                  }));
                }
  ...
  Widget style6BreakNewsData(FeatureSectionModel model) {
    return SizedBox(
        height: MediaQuery.of(context).size.height / 2.8,
        child: SingleChildScrollView(
          controller: style6ScrollController,
          ...
          child: BlocBuilder<SectionByIdCubit, SectionByIdState>(
            builder: (context, state) {
              if (state is SectionByIdFetchSuccess) {
                return Row(
                    children: List.generate(state.totalCount, (index) {
                  BreakingNewsModel data = state.breakNewsModel[index];
                  return SizedBox(
                      width: 180, child: Padding(padding: const EdgeInsets.all(5), child: setImageCard(index: index, model: model, breakingNewsModel: data, breakingNewsList: state.breakNewsModel)));
                }));
              }
              if (state is SectionByIdFetchFailure) {
                return const SizedBox.shrink();
